...
def evaluate_resilience_design(
    ...
    The coupled reliability variables are solved by fixed-point iteration:
        eps_d -> eps_r -> p_u -> eta -> D_theta -> Gamma -> eps_d.

    Gamma can be evaluated using:
      OUTAGE_I_METHOD = "mean" or "empirical"
      OUTAGE_D_METHOD = "mean" or "distribution"
    """
    theta0 = int(theta0)
    theta1 = int(theta1)
    pt = float(pt)
    ...
    outage_i_method = str(_get_param(params, "OUTAGE_I_METHOD", "mean")).lower()
    outage_d_method = str(_get_param(params, "OUTAGE_D_METHOD", "mean")).lower()
    outage_i_sample_cap = int(_get_param(params, "OUTAGE_I_SAMPLE_CAP", 2000))
    outage_i_sample_seed = int(_get_param(params, "OUTAGE_I_SAMPLE_SEED", 123))
    ...
    # Initial eps_d ignores outage contribution. This is only initialization.
    eps_d = (1.0 - pr) * eps_bar

    pu0 = pu1 = eta0 = eta1 = 0.0
    E_D_theta_0 = E_D_theta_1 = 0.0
    outage0: Dict[str, float] = {"Gamma_s": 0.0}
    outage1: Dict[str, float] = {"Gamma_s": 0.0}
    eps_r = 0.0
    ...
    for fp_iter in range(max_fp_iter):
        ...
        if not (0.0 <= eps_d < 1.0):
            return {
                "feasible": False,
                "reason": "eps_d_invalid_in_fixed_point",
                "pt": pt,
                "theta0": theta0,
                "theta1": theta1,
                "eps_bar": eps_bar,
        ...
        outage0 = compute_outage_term_state_semianalytic(
            theta_s=theta0,
            q_ss=q00,
            ...
            weibull_kappa=kappa,
            eta_s=eta0,
            I_samples=predictive_stats.get("I_0_samples", None),
            outage_i_method=outage_i_method,
            outage_d_method=outage_d_method,
            sample_cap=outage_i_sample_cap,
            sample_seed=outage_i_sample_seed,
        )
        outage1 = compute_outage_term_state_semianalytic(
            theta_s=theta1,
            q_ss=q11,
            ...
            weibull_kappa=kappa,
            eta_s=eta1,
            I_samples=predictive_stats.get("I_1_samples", None),
            outage_i_method=outage_i_method,
            outage_d_method=outage_d_method,
            sample_cap=outage_i_sample_cap,
            sample_seed=outage_i_sample_seed + 1,
        )
    ...
    # Recompute p_u from final eps_r once more for exact consistency.
    pu0_info = compute_pu_star(theta_s=theta0, eps_r=eps_r, eps_bar=eps_bar)
    pu1_info = compute_pu_star(theta_s=theta1, eps_r=eps_r, eps_bar=eps_bar)
    if not pu0_info["feasible"] or not pu1_info["feasible"]:
        return {
            "feasible": False,
            "reason": "pu_star_infeasible_final",
            "pt": pt,
            "theta0": theta0,
            "theta1": theta1,
            "eps_bar": eps_bar,
    ...
    # Final recomputation of Gamma with final variables.
    outage0 = compute_outage_term_state_semianalytic(
        theta_s=theta0,
        q_ss=q00,
        E_I_s=float(predictive_stats["E_I_0"]),
...
if __name__ == "__main__":
    ...
    if not solution["feasible"]:
        ...
        print(f"outage D method    : {best['outage_d_method']}")
