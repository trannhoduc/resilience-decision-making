...
def evaluate_resilience_design(
    ...
    Reliability variables are solved by fixed-point iteration:

        eps_d -> eps_r -> p_u -> eta -> D_theta -> Gamma_UB -> eps_d.

    This version uses the fast conservative Option-A outage surrogate:

        Gamma_s^UB(theta_s)
          = min{1, (E[D_theta,s] + E[t_h]) E[1/T_s]}.

    This replaces the older nonlinear term

        E[min{D_theta,s + t_h, T_s - I_s}/T_s]

    by a tractable sufficient upper bound. It avoids the expensive summation over
    the full distribution of I_s, D_theta,s, t_h, and T_s, and avoids the
    optimistic mean-inside-min approximation.
    """
    theta0 = int(theta0)
    theta1 = int(theta1)
    pt = float(pt)
    ...
    # Kept as a parameter for clarity/debugging, but this file uses only Option A.
    outage_constraint_method = str(
        _get_param(params, "OUTAGE_CONSTRAINT_METHOD", "upper_bound_simple")
    ).lower()
    if outage_constraint_method not in {"upper_bound_simple", "option_a"}:
        raise ValueError(
            "This revised file supports OUTAGE_CONSTRAINT_METHOD='upper_bound_simple' only."
        )
    ...
    # Initialization only: ignore outage contribution at the first iterate.
    ...
    # Final consistency pass using the converged eps_d.
    eps_r = compute_eps_r_from_eps_d(eps_l=eps_l, eps_d=eps_d)
    if not (0.0 < eps_r < 1.0):
        return {
            "feasible": False,
    ...
    if not (0.0 < eps_d < eps_l):
        return {
            "feasible": False,
            "reason": "eps_d_out_of_bounds_final",
            "pt": pt,
            "theta0": theta0,
            "theta1": theta1,
            "eps_bar": eps_bar,
